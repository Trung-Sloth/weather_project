document.addEventListener('popstate', () => {
  ...
  db.ref("Toa-do-hien-tai").on("value", (snapshot) => {
    ...
    // console.log("Toa-do-hien-tai: " + lat + ", " + lng);
  ...
  db.ref("Toa-do-hien-tai").on("value", (snapshot) => {
    ...
    if (data && data.lat_cur && data.lng_cur) {
      ...
      if (firebaseMarker) {
        ...
        var prevLat = firebaseMarker.getLatLng().lat;
        var prevLng = firebaseMarker.getLatLng().lng;
        prevMaLat.push(prevLat);
        prevMaLng.push(prevLng);
  ...
  deleteBtn.addEventListener('click', () => {
    ...
    map.removeLayer(currentMarker2);
    map.removeLayer(currentMarker3);
    var arrayLength = prevMaLat.length;
    for (var i = 0; i < arrayLength; i++) {
      map.removeLayer(L.marker([prevMaLat[i], prevMaLng[i]]));
      console.log("Removed previous marker at: " + prevMaLat[i] + ", " + prevMaLng[i]);
    }
